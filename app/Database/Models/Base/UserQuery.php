 ...
 * @method     ChildUserQuery orderByRemember($order = Criteria::ASC) Order by the remember column
 * @method     ChildUserQuery orderByAccountType($order = Criteria::ASC) Order by the account_type column
 ...
 * @method     ChildUserQuery groupByRemember() Group by the remember column
 * @method     ChildUserQuery groupByAccountType() Group by the account_type column
 ...
 * @method     ChildUser findOneByRemember(string $remember) Return the first ChildUser filtered by the remember column
 * @method     ChildUser findOneByAccountType(int $account_type) Return the first ChildUser filtered by the account_type column
 ...
 * @method     ChildUser requireOneByRemember(string $remember) Return the first ChildUser filtered by the remember column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildUser requireOneByAccountType(int $account_type) Return the first ChildUser filtered by the account_type column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUser[]|ObjectCollection findByRemember(string $remember) Return ChildUser objects filtered by the remember column
 * @method     ChildUser[]|ObjectCollection findByAccountType(int $account_type) Return ChildUser objects filtered by the account_type column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'manager', $modelName = '\\Models\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, email, password, remember, account_type, created_at, updated_at FROM user WHERE id = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByRemember('fooValue');   // WHERE remember = 'fooValue'
     * $query->filterByRemember('%fooValue%'); // WHERE remember LIKE '%fooValue%'
     * </code>
     *
     * @param     string $remember The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildUserQuery The current query, for fluid interface
    ...
    public function filterByRemember($remember = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($remember)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $remember)) {
                $remember = str_replace('*', '%', $remember);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(UserTableMap::COL_REMEMBER, $remember, $comparison);
    }
    ...
    public function filterByAccountType($accountType = null, $comparison = null)
    {
        if (is_array($accountType)) {
            ...
            if (isset($accountType['min'])) {
                $this->addUsingAlias(UserTableMap::COL_ACCOUNT_TYPE, $accountType['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($accountType['max'])) {
                $this->addUsingAlias(UserTableMap::COL_ACCOUNT_TYPE, $accountType['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_ACCOUNT_TYPE, $accountType, $comparison);
    }
